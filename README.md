...
**Current development (September 20): E120 confirms an end-turn power-order mismatch.** With Runic Cube, Combust followed by Battle Trance draws an extra card in E116 but not in the original; the reverse order and two controls match. The source refresh is stopped with 1,944 files preserved and ten original routes matched. Training waits for a repair that retains power acquisition order. E116's Bomb repair remains separately scoped evidence. The 50% unseen-seed goal remains open. [Diagnostic](docs/experiments/e120-end-turn-order-diagnostic.json), [current status](docs/ironclad-training-status.md).
