...
**Current development (September 21): E127 first-relic data passed independent audits; conditional-card preparation is running.** All 17,552 continuations, 2,552,610 outside choices and 4,388 parent controls passed with zero execution faults. The complete audit and branch hashes were rechecked. Joint labels and E128 training/validation remain pending; these counts are not a policy gain. The 625/6,144 known-family source baseline remains simulator evidence. [Relic-stage completion](docs/experiments/e127-first-relic-completion.json), [current status](docs/ironclad-training-status.md).
