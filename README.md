...
**Current development (September 20): E121 retains player power order; full-run admission is pending.** The E120 controlled mismatch is repaired: all four recorded original sequences match. 279 local checks and 47 clean patch-build checks pass; snapshots and copies retain order. One preselected development route is registered for natural, original and RNG integration. Training and source collection wait for that gate. E117 stays stopped, E118/E119 closed; the 50% unseen goal remains open. [Preparation](docs/experiments/e121-repair-preparation.json), [current status](docs/ironclad-training-status.md).
