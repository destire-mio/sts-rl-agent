...
**September 21 scope update:** complete the current collection, fits and validation, then review the training approach before selecting another round. The requested 50 original-game runs of the 10.17% parent baseline are blocked before launch by the live-policy interface: 0/50 completed, no native win-rate estimate. The legacy live entry is incompatible, and selected simulator-route replay is not natural evaluation. Original-rule alignment remains paused. [Capability check](docs/experiments/e130-native-baseline-capability.json).

**Current development (September 20): simulator source verification is complete, and E127 continuation collection is running.** E129 completed the interrupted audits and admitted all 6,144 E122 natural games: 834,427 outside choices, 6,144 terminal/state/RNG replays and 625 winner replans passed. At 21:50 Shanghai, the first-relic collection had completed 617/17,552 jobs; eight workers were OS-verified live. Card combinations and the fixed 1,536-versus-4,608-family fits follow their separate label gates. Optimizer updates remain zero at this checkpoint. The original source deadline failure stays recorded; original-game alignment remains paused. The 625/6,144 source baseline is known-family simulator evidence, not a new learned gain or unseen acceptance. [Source completion](docs/experiments/e122-e129-source-completion.json), [recovery launch](docs/experiments/e129-audit-recovery-launch.json).
