...
**Current development (September 20): E98 repairs the Guardian HP/block difference that stopped E97.** Seven original controls, 214 regressions and the exact natural failure boundary pass. The preselected seed replans to Heart victory and its full original route matches 1,115 commands at 49HP, with terminal/RNG, 194 outside-NN choices and repeat-planning checks. No new model was trained. E97 remains stopped; affected sources and labels require a new repaired-engine registration. The data-scale hypothesis and 50% unseen-win goal remain open. [Repair evidence](sim_patch/alignment/e98-guardian-queue-report.json), [preserved E97 stop](docs/experiments/e97-source-stop-result.json), [current status](docs/ironclad-training-status.md). E99 registers regeneration of the fixed 6,144 small/expanded study sources under E98, with eight workers and 20-minute progress checks. [Source-refresh protocol](docs/experiments/e99-source-refresh-protocol.json).
