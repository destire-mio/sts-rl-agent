"""Launch bounded paired-afterstate fitting after root data and input review."""
import argparse
...
def main(root, phase, owned):
    ...
    import heart_paired_afterstate_value as I
    E = I.E
    ...
    if not owned:
        if phase == 'train':
            ...
            assert reviewed['data_completion_sha256'] == E.sha(root / 'data/completion.json')
        control.mkdir()
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location('e176_owned', launcher)
        ...
        outcome = owner.run_owned(output, [sys.executable, '-u', str(root / 'program/heart_paired_afterstate_value.py'), phase, '--study', str(root)],
