...
def main(root):
    ...
    for fold in range(3):
        ...
        expected_initial = I.new_model(store.spec['width'] + store.spec['descriptor_dim'], fold).state_dict()
        ...
        assert cp['provenance']['exact_values_sha256'] == E.sha(Path(plan['diagnosis']) / 'exact-observed-values.npz')
    ...
    review = dict(status='complete_reviewed', experiment='E160', selected_steps=steps,
