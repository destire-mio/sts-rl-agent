...
def main(root):
    ...
    for fold in range(3):
        ...
        initial = V.warm_model(encoder, width, fold, True).state_dict()
        ...
        for at in range(0, len(validation_ids), 128):
            ...
            np.testing.assert_allclose(actual.to_dense().numpy(), features, atol=0, rtol=0)
        ...
        assert cp['model_type'] == 'empty_reward_parent_value' and cp['normalization'] == L.VERSION and cp['prediction_only']
    ...
    result = dict(status='complete_reviewed', experiment='E185', result=report, inner_checkpoints_verified=checkpoints,
        ...
        process_cleanup_verified=True, learning_completion_sha256=E.sha(root/'learning/completion.json'),
