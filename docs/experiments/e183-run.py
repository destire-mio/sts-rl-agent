"""Own the bounded whole-policy search comparison."""
import argparse
...
def main(root, owned):
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location('e183_owned', launcher)
        owner = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(owner)
        assert Path(owner.C.__file__).resolve() == launcher.parent / 'run_collections.py'
        output = root / 'search-execution'
        output.mkdir()
        E.write(control / 'started.json', dict(pid=os.getpid(), at=datetime.now(timezone.utc).isoformat()))
        result = owner.run_owned(output, [sys.executable, '-u', str(root / 'program/heart_whole_policy_search.py'), '--study', str(root)],
        ...
        E.proof(root / 'search', 'completion.json')
        end = dict(status='complete', phase='search', exit_code=0,
            owned_exit_sha256=E.sha(output / 'pipeline-process-exit.json'), completion_sha256=E.sha(root / 'search/completion.json'))
    except BaseException:
        end = dict(status='stopped_with_error', phase='search', exit_code=1, error=traceback.format_exc())
        traceback.print_exc()
