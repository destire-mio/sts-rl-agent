"""Own E139 boss-conditioned fit-only comparison and native checks."""
...
def main():
    ...
    import heart_boss_conditioned_experiment as F
    E = F.E
    outcomes = []
    try:
        ...
        plan = F.registered(STUDY)
        launcher = Path(control['owned_launcher'])
        sys.path.insert(0, str(launcher.parent))
        spec = importlib.util.spec_from_file_location('e139_owned', launcher)
        ...
        for name, budget in [('train', plan['resources']['training_seconds']),
                             ('verify', plan['resources']['verification_seconds'])]:
            F.registered(STUDY)
            ...
            result = owner.run_owned(directory, [sys.executable, '-u',
                str(STUDY/'program/heart_boss_conditioned_experiment.py'), name, '--study', str(STUDY)],
        ...
        evidence = STUDY/'result/completion-verification.json'
