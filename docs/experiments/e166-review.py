...
def comparison(a, b):
    delta = a[:, 0] - b[:, 0]
    return np.column_stack((-delta, .5 * (a[:, 1] + b[:, 1]), delta))
...
def main(root):
    ...
    for fold in range(3):
        ...
        expected_initial = S.warm_model(Path(plan['encoder_source']), store.spec['width'] + store.spec['descriptor_dim'], fold, True).state_dict()
        ...
        for at in range(0, len(ids), 128):
            ...
            reference = matrix(store, store.edge_state[edges], store.parent[store.edge_state[edges]])
            target = labels[chosen]
            for step, weight in weights.items():
                predicted = comparison(logits(weight, features), logits(weight, reference)).astype(np.float64)
        ...
        assert cp['provenance']['encoder_sha256'] == E.sha(Path(plan['encoder_source']) / 'learning' / f'fold-{fold}/auxiliary.pt')
        ...
        for at in range(0, len(states), 64):
            ...
            parent_rows = np.repeat(ptr[:-1] + store.parent[batch] - store.menu_ptr[batch], sizes)
            prediction = comparison(prediction, prediction[parent_rows])
    ...
    assert report['actor_optimizer_steps'] == 6000 + sum(steps)
    review = dict(status='complete_reviewed', experiment='E166', selected_steps=steps,
        actor_optimizer_steps=report['actor_optimizer_steps'], inner_checkpoints_verified=checkpoints,
