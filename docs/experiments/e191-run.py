"""Own the bounded whole-policy gradient comparison."""
import argparse
...
def main(root, owned):
    ...
    import heart_whole_policy_gradient as L
    E = L.E
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location('e191_owned', launcher)
        owner = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(owner)
        assert Path(owner.C.__file__).resolve() == launcher.parent / 'run_collections.py'
        output = root / 'train-execution'
        output.mkdir()
        E.write(control / 'started.json', dict(pid=os.getpid(), at=datetime.now(timezone.utc).isoformat()))
        result = owner.run_owned(output, [sys.executable, '-u', str(root / 'program/heart_whole_policy_gradient.py'), '--study', str(root)],
        ...
        end = dict(status='complete', phase='train', exit_code=0,
            owned_exit_sha256=E.sha(output / 'pipeline-process-exit.json'), completion_sha256=E.sha(root / 'learning/completion.json'))
    except BaseException:
        end = dict(status='stopped_with_error', phase='train', exit_code=1, error=traceback.format_exc())
        traceback.print_exc()
