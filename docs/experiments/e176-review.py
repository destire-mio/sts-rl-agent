...
def main(root):
    ...
    for fold in range(3):
        ...
        assert [r['step'] for r in curve] == plan['recipe']['checkpoints']
        checked_curve = []
        ...
        assert checkpoint['provenance'] == dict(fold=fold, fit_families=roles['fit'], selected_steps=selected,
            value_sha256=E.sha(source / 'learning' / f'fold-{fold}/value.pt'), recipe=plan['recipe'])
    ...
    rng = np.random.default_rng(plan['recipe']['bootstrap_seed'])
    means = np.empty(plan['recipe']['bootstrap_draws'])
    ...
    updates = 3*plan['recipe']['steps']+sum(r['selected_steps'] for r in folds)
    assert updates == actual['value_optimizer_updates']
    result = dict(status='complete_reviewed', experiment='E176', folds=folds,
