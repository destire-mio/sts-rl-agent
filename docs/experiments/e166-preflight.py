...
def main(root):
    ...
    model = S.warm_model(Path(plan['encoder_source']), store.spec['width'] + store.spec['descriptor_dim'], 0, True)
    S.fit(model, examples, inner, 8, 0)
    ...
    policy = S.StructuredPolicy(cp, x)
