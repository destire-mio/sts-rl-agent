...
def main(root):
    ...
    for fold in range(3):
        ...
        previous = Path(plan['preceding_study']) / 'learning' / f'fold-{fold}'
        for step in V.RECIPE['checkpoints']:
            old_weights = torch.load(previous / f'inner-{step}.pt', weights_only=True, map_location='cpu')
            assert all(torch.equal(value, old_weights[key]) for key, value in weights[step].items())
    ...
    result = dict(status='complete_reviewed', experiment='E170', folds=results, held_brier=neural,
        ...
        original_optimizer_prefix_identical_through_5000=True,
