"""E117 source gate: all assigned development winners, one owned JVM at a time."""
import argparse
...
SOURCE = REPO / 'runs/heart-e116-scale-source-refresh-20260920-01/natural'
...
def helper():
    spec = importlib.util.spec_from_file_location('e117_original_scope_auditor', HELPER)
...
def prepare(deadline):
    ...
    plan = {'experiment': 'E117P', 'created_at': datetime.now(timezone.utc).isoformat(),
...
def run():
    ...
    try:
        ...
        proof = {'experiment': 'E117P', 'status': 'complete' if complete else 'stopped_with_unresolved_findings',
