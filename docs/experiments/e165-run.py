...
def main(root, phase, owned):
    ...
    import heart_combat_local_inputs as I
    E = I.E
    ...
    if not owned:
        if phase == 'train':
            ...
            assert review['data_completion_sha256'] == E.sha(root / 'data/completion.json')
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location('e165_owned', launcher)
        ...
        outcome = owner.run_owned(output, [sys.executable, '-u', str(root / 'program/heart_combat_local_inputs.py'), phase, '--study', str(root)],
