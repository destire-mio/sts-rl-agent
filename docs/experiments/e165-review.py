...
def main(root):
    ...
    spec = importlib.util.spec_from_file_location('heart_review', Path(__file__).with_name('e165-heart-review.py'))
    ...
    for fold in range(3):
        ...
        assert all(torch.all(w['input.weight'][:, plan['masked_columns']] == 0) for w in weights.values())
        initial = L.local_model(store.spec['width'] + store.spec['descriptor_dim'], fold, plan['masked_columns']).state_dict()
        ...
        assert torch.all(final['input.weight'][:, plan['masked_columns']] == 0)
    ...
    if passed:
        ...
        E.write(root / 'training-review.json', dict(status='complete_reviewed', experiment='E165', auxiliary=auxiliary,
