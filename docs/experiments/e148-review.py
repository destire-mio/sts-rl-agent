"""Rebuild E148 input rows and held-family decisions with NumPy inference."""
import argparse
...
def review(root):
    ...
    E.require([r['stage'] for r in end['stages']]==['build','train'],'wrong executed stages')
    ...
    E.proof(root/'store','completion.json');E.proof(root/'learning','completion.json')
    meta=E.read(root/'store/metadata.json');spec=meta['spec'];rows=meta['states']
    ...
    for arm in R.ARMS:
        gathered={}
        for held in range(3):
            directory=root/'learning'/f'{arm}-fold-{held}';cp=torch.load(directory/'candidate.pt',weights_only=True,map_location='cpu')
            ...
            E.require(cp['support']==support and cp['feature_spec']==spec and cp['provenance']['fit_families']==fit and
                      ...
                      cp['provenance']['store_sha256']==E.sha(root/'store/completion.json'),'checkpoint provenance differs')
        ...
        ordered=[gathered[s] for s in roles];E.require(ordered==E.read(root/'learning'/f'{arm}-out-of-fold.json'),'aggregate order differs')
        counts={s:m.paired([r['parent'] for r in ordered],[r['targets'][s] for r in ordered]) for s in R.SCOPES}
        E.require(counts==report['arms'][arm]['counts'],'independent paired counts differ')
        passed=all(counts[s]['net_gain']>=20 and counts[s]['exact_p']<.025 for s in ('first_card','boss_then_card'))
        E.require(passed==report['arms'][arm]['screen_passed'],'screen differs')
        reviewed[arm]=dict(counts=counts,screen_passed=passed)
    result=dict(status='complete_recorded_screen_not_adopted',experiment='E148',at=datetime.now(timezone.utc).isoformat(),
        arms=reviewed,states=7448,families=1536,numpy_state_inferences=verified_states,raw_terminal_files=len(cache),
        stage_counts=dict(stage_counts),mixed_outcome_states=dict(mixed),optimizer_updates=12000,
