"""E101D natural paired development; original verification precedes selection."""
import argparse
...
def natural(arm):
    ...
    try:
        ...
        rows = H.run_jobs(folder, jobs, config, 'E101D_' + arm + '_natural', deadline, worker_fn=D.C.worker)
        ...
        repeated = H.run_jobs(folder, repeat_jobs, config, 'E101D_' + arm + '_winner_replans',
...
def prepare_original(arm):
    plan, source, report = natural_ready(arm)
    assert report['passed'], 'natural screen failed; no original candidate admission'
    destination = ROOT / 'original-candidates' / arm
    assert not destination.exists(), 'preserve previous original attempt'
    registration = {'experiment': 'E101P', 'arm': arm, 'source_runtime': str(source),
        'identity': report['identity'], 'development_seeds': T.assigned_roles()[1]['train_development'],
        'source_manifest_sha256': sha(source / 'manifest.json'),
        'source_completion_sha256': sha(source / 'completion-verification.json'),
        'source_report_sha256': sha(source / 'report.json'),
        'parent_registration_sha256': sha(ROOT / 'development-registration.json'),
        'reference_game_sha256': plan['reference_game_sha256'],
        'harness_sha256': read(ROOT / 'development-registration.json')['original_harness_sha256'],
        'selection': plan['original_selection'], 'verification': plan['original_verification'],
        'resources': {'stage_seconds': 10800, 'per_original_seconds': 900, 'observer_seconds': 1200},
        'limits': 'All winning routes of this screened development arm. No unseen success claim or losing-route population parity.'}
    write(destination / 'registration.json', registration)
    return destination
...
def require_original_complete(proof, expected, identity):
    assert proof['status'] == 'complete'
    assert proof['identity'] == identity
    assert len(expected) == len(set(expected))
    assert proof['matched_routes'] == proof['attempted'] == proof['requested_winning_routes'] == len(expected)
    assert proof['unattempted_seeds'] == [] and [r['seed'] for r in proof['rows']] == expected
    assert all(r['status'] == 'matched' for r in proof['rows'])
...
def finalize():
    ...
    for arm in ARMS:
        ...
        if not report['passed']:
            continue
        original = ROOT / 'original-candidates' / arm
        proof = T.proof(original, 'completion-verification.json')
        expected = sorted(r['seed'] for r in read(source / 'source-index.json') if r['status'] == 'heart_win')
        require_original_complete(proof, expected, report['identity'])
        reports[arm]['original_gate_completed'] = True
        hashes[str(original / 'completion-verification.json')] = sha(original / 'completion-verification.json')
    ...
    write(OUTPUT / 'development-decision.json', {'status': 'complete', 'selected_arm': selected,
        ...
        'arms': reports, 'production_adoption': False, 'unseen_acceptance_games': 0,
...
if __name__ == '__main__':
    ...
    parser.add_argument('command', choices=('check-gates', 'natural', 'prepare-original', 'finalize'))
    ...
    if args.command == 'check-gates': learning_ready()
        ...
        if args.command == 'natural': natural(args.arm)
        else: print(prepare_original(args.arm))
