"""Inspect every retained E160 inner model without fitting or selecting a policy."""
import argparse
...
def main(source, output):
    ...
    E, O = I.E, I.O
    plan = I.registered(source)
    assert E.read(source / 'training-review.json')['status'] == 'complete_reviewed'
    E.proof(source / 'learning', 'completion.json')
    spec = importlib.util.spec_from_file_location('review', Path(__file__).with_name('e160-review.py'))
    ...
    result = dict(status='complete', experiment='E161', source=str(source), curves=report,
        ...
        limits='Uses only inner fitting/validation families, not the excluded outer fold. Counts refer to branch decisions, not games. Unknown outcomes use0 only for a stated pessimistic lower bound, never a recorded death. This diagnosis does not change E160 stopping or adopt any saved model.')
