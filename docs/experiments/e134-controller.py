"""Own the zero-new-sampling E134 fit and native replay-check stages."""
...
def main():
    ...
    try:
        ...
        plan = E.registered(STUDY)
        launcher = Path(control['owned_launcher'])
        sys.path.insert(0, str(launcher.parent))
        spec = importlib.util.spec_from_file_location('e134_owned', launcher)
        ...
        for name, budget in [('train', plan['resources']['fit_seconds']),
            ...
            result = owner.run_owned(directory, [sys.executable, '-u',
                str(STUDY/'program/heart_existing_data_experiment.py'), name, '--study', str(STUDY)],
