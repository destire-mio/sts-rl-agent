...
def main(root):
    ...
    for fold in range(3):
        ...
        rng = np.random.default_rng(V.RECIPE['seed']+2000+fold)
        indices = {}
        for key, families in (('validation_ids', valid), ('held_ids', held)):
            draws = rng.random((V.RECIPE['validation_draws'], 2))
            ids = []
            for a, b in draws:
        ...
        assert [r['step'] for r in curve] == V.RECIPE['checkpoints']
        ...
        assert provenance['selected_steps'] == selected and provenance['recipe'] == V.RECIPE
    ...
    updates = 15000+sum(r['selected_steps'] for r in results)
    assert updates == report['value_optimizer_updates']
    result = dict(status='complete_reviewed', experiment='E169', folds=results, held_brier=neural,
        baseline_brier=reference, prediction_gate_passed=passed, value_optimizer_updates=updates,
        independent_inner_checkpoints=checkpoint_count, independent_held_predictions=3*V.RECIPE['validation_draws'],
