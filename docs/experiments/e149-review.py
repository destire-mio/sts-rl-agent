"""Check E149 masked weights, source-state NumPy choices and paired outcomes."""
import argparse
...
def review(root):
    ...
    for arm in ('within_state_no_map',):
        gathered={}
        for held in range(3):
            ...
            E.require(cp['masked_columns']==ablation['masked_columns'] and not bool(cp['value_state']['input.weight'][:,cp['masked_columns']].count_nonzero()),'map input weights not zero')
        ...
        E.require(comparison==report['against_E148_with_map'],'matched ablation counts differ')
        reviewed[arm]=dict(counts=counts,screen_passed=passed,against_E148_with_map=comparison)
    result=dict(status='complete_recorded_screen_not_adopted',experiment='E149',at=datetime.now(timezone.utc).isoformat(),
