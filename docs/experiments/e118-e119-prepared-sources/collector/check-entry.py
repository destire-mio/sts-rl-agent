"""Check E118's gated entry and replay one already verified E116 source."""
from pathlib import Path
import ast, hashlib, importlib.util, json, sys

C = Path(__file__).resolve().parent
N = C.parent / 'heart-e116-scale-source-refresh-20260920-01'
R = C.parents[1]
S = R.parent / 'ironclad-alignment/evidence/e116-bomb-instance-repair-20260920-01/candidate'
sha = lambda p: hashlib.sha256(Path(p).read_bytes()).hexdigest()
read = lambda p: json.loads(p.read_text())
spec = importlib.util.spec_from_file_location('e118_collector', C / 'run_collections.py')
...
output = {
    ...
    'known_E116_source_replay': {'seed': row['seed'], 'episode_sha256': sha(path),
