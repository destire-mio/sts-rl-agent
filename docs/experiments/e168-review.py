...
def main(root):
    ...
    for fold in range(3):
        ...
        expected_initial = S.warm_model(Path(plan['encoder_source']), store.spec['width'] + store.spec['descriptor_dim'], fold, True, plan['item_layout']).state_dict()
        ...
        for at in range(0, len(ids), 128):
            ...
            for step, weight in weights.items():
                predicted = logits(weight, S.numpy_route(features, plan['item_layout'])).astype(np.float64)
        ...
        assert cp['model_type'] == S.MODEL_TYPE and cp['item_layout'] == plan['item_layout']
        ...
        for at in range(0, len(states), 64):
            ...
            prediction = logits(cp['actor_state'], S.numpy_route(matrix(store, np.repeat(batch, sizes), actions), plan['item_layout']))
    ...
    review = dict(status='complete_reviewed', experiment='E168', selected_steps=steps,
