"""Bounded E116 integration; stages advance on completion, observations every 20 minutes."""
...
def original_plan():
    ...
    N.mkdir()
    write(N / 'plan.json', {
        'experiment': 'E116-original-integration', 'registered_at': datetime.now(timezone.utc).isoformat(),
        ...
        'selection': 'Preselected E116 known route; no replacement or change to outcome.',
        'scope': 'Full natural original replay plus independent Bomb-instance comparison; not unseen evaluation.',
...
def native_or_recorded(mode):
    ...
    write(Q / (mode + '-bomb-comparison.json'), {
...
def pipeline():
    ...
    write(Q / 'integration-stages-complete.json', {
        ...
        'original_bomb_comparison_sha256': sha(Q / 'original-bomb-comparison.json'),
        'recorded_bomb_comparison_sha256': sha(Q / 'recorded-bomb-comparison.json'),
