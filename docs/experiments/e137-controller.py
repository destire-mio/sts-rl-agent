"""Own gated E137 first-card fitting and native choice checks."""
...
def main():
    ...
    import heart_first_card_training as F
    E = F.E
    outcomes = []
    try:
        ...
        spec = importlib.util.spec_from_file_location('e137_owned', launcher)
        ...
        for name, budget in [('train', plan['resources']['training_seconds']),
            ...
            result = owner.run_owned(directory, [sys.executable, '-u',
                str(STUDY/'program/heart_first_card_training.py'), name, '--study', str(STUDY)],
        ...
        evidence = STUDY/'learning/completion-verification.json'
