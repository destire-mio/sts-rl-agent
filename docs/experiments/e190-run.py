"""Launch bounded direct-combat auxiliary fitting after root data and input review."""
import argparse
...
def main(root, phase, owned):
    ...
    import heart_direct_combat_delta as I
    E = I.E
    ...
    if not owned:
        if phase == 'train':
            ...
            assert reviewed['target_completion_sha256'] == E.sha(Path(plan['target_source']) / 'data/completion.json')
        control.mkdir()
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location('e190_owned', launcher)
        owner = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(owner)
        assert Path(owner.C.__file__).resolve() == launcher.parent / 'run_collections.py'
        output = root / (phase + '-execution')
        output.mkdir()
        budget = plan['phase_timeout_seconds']
        E.write(control / 'started.json', dict(pid=os.getpid(), at=datetime.now(timezone.utc).isoformat(), phase=phase))
        outcome = owner.run_owned(output, [sys.executable, '-u', str(root / 'program/heart_direct_combat_delta.py'), phase, '--study', str(root)],
        ...
        proof = root / ('learning/completion.json' if phase == 'train' else 'data/completion.json')
