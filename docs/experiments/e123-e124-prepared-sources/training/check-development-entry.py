...
def main():
    ...
    order_checker_sha = 'fixture-order-checker'
    ...
    checks['original_admission_negative_cases'] = {key: rejected(
        lambda v=value: G.require_original_complete(v, [11, 29], identity, helper_sha, combiner_sha, checker_sha, order_checker_sha)) for key, value in negatives.items()}
    P.mkdir()
    ...
    C = G.T.COLLECTOR
    spec = importlib.util.spec_from_file_location('e124d_probe_collector', C / 'run_collections.py')
    ...
    original_source = G.ROOT.parents[2] / 'ironclad-alignment/evidence/e122-development-parity-20260920-01/cohort.py'
    import ast
    ...
    write(P / 'completion-verification.json', {'status': 'passed', 'checks': checks,
        ...
        'limits': 'Software entry/selection/whole-route audit controls on the known E121 parent route with a zero-head fixture; no learned candidate or original cohort validation.',
