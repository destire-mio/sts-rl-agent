"""E124D natural paired development; original verification precedes selection."""
import argparse
...
def registered():
    ...
    for path, expected in reg['original_harness_sha256'].items():
        assert sha(path) == expected, path
...
def natural(arm):
    ...
    try:
        ...
        rows = H.run_jobs(folder, jobs, config, 'E124D_' + arm + '_natural', deadline, worker_fn=D.C.worker)
        ...
        repeated = H.run_jobs(folder, repeat_jobs, config, 'E124D_' + arm + '_winner_replans',
...
def prepare_original(arm):
    ...
    registration = {'experiment': 'E124P', 'arm': arm, 'source_runtime': str(source),
