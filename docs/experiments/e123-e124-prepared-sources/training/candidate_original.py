"""E124P candidate gate: all development winners, one owned JVM at a time."""
import argparse
...
def helper():
    spec = importlib.util.spec_from_file_location('e124_original_scope_auditor', HELPER)
...
def prepare(deadline):
    ...
    plan = {'experiment': 'E124P', 'created_at': datetime.now(timezone.utc).isoformat(),
...
def one(seed):
    ...
    if original['status'] == 'original_heart_trace_matched':
        ...
        instance_proof = {'status': 'matched' if recorded['status'] == 'recorded_original_heart_trace_matched' else 'failed',
            ...
            'combiner_sha256': sha(G.ROOT / 'turn_extras.py'), 'instance_checker_sha256': sha(G.ROOT / 'bomb_instances.py'),
        ...
        order_proof = {'status': instance_proof['status'], 'seed': seed,
            'engine_sha256': reg['identity']['engine_sha256'],
            'combiner_sha256': sha(G.ROOT / 'turn_extras.py'), 'order_checker_sha256': sha(G.ROOT / 'power_order.py'),
    ...
    try:
        ...
        for result in results:
            if result['status'] == 'matched':
                ...
                assert instance_proof['combiner_sha256'] == sha(G.ROOT / 'turn_extras.py')
        ...
        proof = {'experiment': 'E124P', 'status': 'complete' if complete else 'stopped_with_unresolved_findings',
