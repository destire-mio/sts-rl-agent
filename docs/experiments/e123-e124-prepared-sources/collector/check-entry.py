"""Check E123's gated entry and replay one already verified E121 source."""
from pathlib import Path
import ast, hashlib, importlib.util, json, sys

C = Path(__file__).resolve().parent
N = C.parent / 'heart-e121-scale-source-refresh-20260920-01'
R = C.parents[1]
S = R.parent / 'ironclad-alignment/evidence/e121-power-order-repair-20260920-01/candidate'
sha = lambda p: hashlib.sha256(Path(p).read_bytes()).hexdigest()
read = lambda p: json.loads(p.read_text())
spec = importlib.util.spec_from_file_location('e123_collector', C / 'run_collections.py')
...
path = S / 'episodes/1138994370.json.gz'
...
assert node is not None and node['seed'] == 1138994370
...
output = {
    ...
    'known_E121_source_replay': {'seed': row['seed'], 'episode_sha256': sha(path),
