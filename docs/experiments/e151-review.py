"""Check E151 nested roles, stopping choices, loss formula and NumPy decisions."""
import argparse
...
def review(root):
    ...
    import heart_nested_stopping as M
    ablation,control_study=M.registered(root)
    source=Path(ablation['data_source'])
    E.proof(control_study/'learning','completion.json')
    ...
    def features(state):
        ...
        return a
    def scored(cp,indices,raw=None):
        ...
        for at in range(0,len(indices),128):
            ...
            for i,a in zip(chunk,arrays):
                ...
                if raw is not None:raw[row['state_id']]=v.copy()
    ...
    for arm in ('nested_stopped_parent_prior',):
        gathered={}
        for held in range(3):
            ...
            outer_fit=[f for f in meta['families'] if f['seed'] in fitset]
            inner_fit,inner_valid=M.inner_partition(outer_fit)
            recorded_roles=E.read(directory/'inner-roles.json')
            E.require(recorded_roles==dict(fit=[f['seed'] for f in inner_fit],validation=[f['seed'] for f in inner_valid],outer_validation=val),'inner/outer roles differ')
            stopping=E.read(directory/'stopping-choice.json');curve=stopping['curve']
            E.require([r['step'] for r in curve]==list(M.CHECKPOINTS) and all(np.isfinite(r['validation_loss']) for r in curve),'stopping curve incomplete')
            selected=min(curve,key=lambda r:(r['validation_loss'],r['step']))['step']
            E.require(selected==stopping['selected_step'] and cp['provenance']['stopping_choice_sha256']==E.sha(directory/'stopping-choice.json'),'stopping selection differs')
            inner_state=torch.load(directory/'inner-final.pt',weights_only=True,map_location='cpu')
            inner_support=sorted({s for row in rows if row['seed'] in {f['seed'] for f in inner_fit} for s in row['support']})
            inner_indices=[i for f in inner_valid for stage in f['stages'] for i in stage];raw={}
            scored(dict(value_state=inner_state,support=inner_support),inner_indices,raw)
            family_losses=[]
            for f in inner_valid:
                stage_losses=[]
                for stage in f['stages']:
                    loss=[]
                    for i in stage:
                        row=rows[i];q=raw[row['state_id']].astype(np.float64);y=np.asarray(row['targets'])
                        error=q-y;loss.append(float(np.mean((error-error.mean())**2)))
                    stage_losses.append(float(np.mean(loss)))
                family_losses.append(float(np.mean(stage_losses)))
            E.require(abs(float(np.mean(family_losses))-curve[-1]['validation_loss'])<1e-7,'independent inner loss differs')
            if selected==2000:
                original=torch.load(control_study/'learning'/f'fold-{held}/candidate.pt',weights_only=True,map_location='cpu')
                E.require(all(torch.equal(v,original['value_state'][k]) for k,v in cp['value_state'].items()),'fixed2000 control tensors differ')
            ...
            E.require(cp['support']==support and cp['feature_spec']==spec and cp['provenance']['fit_families']==fit and
                      ...
                      cp['provenance']['optimizer_updates']==selected and
        ...
        control=E.read(control_study/'learning/out-of-fold.json')
        ...
        E.require(comparison==report['against_E150'],'matched ablation counts differ')
        reviewed[arm]=dict(counts=counts,screen_passed=passed,against_E150=comparison)
    result=dict(status='complete_recorded_screen_not_adopted',experiment='E151',at=datetime.now(timezone.utc).isoformat(),
        arms=reviewed,states=7448,families=1536,numpy_state_inferences=verified_states,raw_terminal_files=len(cache),
        stage_counts=dict(stage_counts),mixed_outcome_states=dict(mixed),optimizer_updates=report['optimizer_updates'],selected_steps=report['selected_steps'],
