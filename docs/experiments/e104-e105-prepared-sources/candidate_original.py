"""E105P candidate gate: all development winners, one owned JVM at a time."""
import argparse
...
def helper():
    spec = importlib.util.spec_from_file_location('e105_original_scope_auditor', HELPER)
...
def prepare(deadline):
    ...
    plan = {'experiment': 'E105P', 'created_at': datetime.now(timezone.utc).isoformat(),
...
def run():
    ...
    try:
        ...
        proof = {'experiment': 'E105P', 'status': 'complete' if complete else 'stopped_with_unresolved_findings',
