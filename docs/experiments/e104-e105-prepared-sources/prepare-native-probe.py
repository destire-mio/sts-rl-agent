...
(P / 'plan.json').write_text(json.dumps({
    ...
    'episode': str(S / 'episodes/192246470.json.gz'),
    'episode_sha256': sha(S / 'episodes/192246470.json.gz'),
    'runtime_manifest_sha256': sha(N / 'manifest.json'),
    'collector_registration_sha256': sha(C / 'registration.json'),
    'scope': 'Two zero-head readout models on the one observed E102 parent relic/card path; use only its observed parent terminal, no counterfactual labels or complete-tree claim. Positive native choices plus wrong-expanded-relic/card expectation rejections. No MCTS or optimizer updates.'}, indent=2) + '\n')
