"""Own gated E143 historical trajectory re-encoding."""
...
def main():
    ...
    import heart_continuous_data as F
    E = F.E
    outcomes = []
    try:
        ...
        spec = importlib.util.spec_from_file_location('e143_owned', launcher)
        ...
        for name, budget in [('extract', plan['timeout_seconds']+120)]:
            ...
            result = owner.run_owned(directory, [sys.executable, '-u',
                str(STUDY/'program/heart_continuous_data.py'), name, '--study', str(STUDY)],
