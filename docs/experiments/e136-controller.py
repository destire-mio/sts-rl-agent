"""Own E136 preparation and existing-route audits; no new games."""
...
def main():
    ...
    import heart_first_card_reuse as F
    E = F.E
    outcomes = []
    try:
        ...
        spec = importlib.util.spec_from_file_location('e136_owned', launcher)
        ...
        for name, budget in [('prepare', 3600), ('audit', plan['audit_seconds'])]:
            F.registered(STUDY)
            ...
            result = owner.run_owned(directory, [sys.executable, '-u',
                str(STUDY/'program/heart_first_card_reuse.py'), name, '--study', str(STUDY)],
