...
`replay_recorded_winner.py` rechecks identical natural action/timing paths against immutable original responses using the pinned live comparator and additional run-only RNG checks during combat. It performs no new JVM run and never imports original state. Changed paths require new original execution. Eight known-route/negative cases and six input-integrity checks validate this boundary; source records and licensed oracle dependencies remain local. See [E83 evidence](e83-recorded-replay-report.json).


E85 (`e85_dropkick.patch`, after `e81_sever_soul.patch`) captures Dropkick damage at card use, retaining Vigor before it is consumed. E86 (`e86_discard_copy.patch`, after E85) restores the temporary cost of copies generated by MakeTempCardInDiscard. Generic discard, hand generation and overflow paths retain their original behavior. E86 full CTest passes 204 cases, including six focused groups; an independently applied build passes those six. Seven controlled original cases and the natural divergence boundary match. Two known-seed full original routes cover 1,371/1,388 commands. [E86 report](e86-discard-copy-report.json), [source manifest](e86-discard-copy-manifest.json).

`e86-discard-observer.patch` waits for native ShowCardAndAddToDiscardEffect callbacks and records waiting frames. It assigns no card cost or RNG. Earlier recorded observations retain their observer identity; a cached comparison at an older unsettled boundary requires a new original run rather than ignoring the discrepancy. E87 regenerates all assigned families under the E86 engine. Old frozen labels remain separate; [current status](../../docs/ironclad-training-status.md) supersedes the dated milestones above.
...
[Patch manifest](e98-guardian-queue-manifest.json), [result](e98-guardian-queue-report.json), [regressions](tests/e98_guardian_queue.cpp).
...
[Patch manifest](e102-spot-weakness-manifest.json), [result](e102-spot-weakness-report.json), [regressions](tests/e102_spot_weakness.cpp).
...
[Patch manifest](e106-unceasing-top-manifest.json), [result](e106-unceasing-top-report.json), [regressions](tests/e106_unceasing_top.cpp).
