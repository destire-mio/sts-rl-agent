...
**2026-09-20 开发状态：原版对齐暂停，E129 补齐模拟器校验后接回 E127／E128 训练。** E122 完成 6144 局采样和 625 条胜局重规划，三小时共用额度在末段校验时耗尽：38 批通过、8 批中断、146 批未启动。21:30 系统核对确认恢复进程运行，剩余校验使用一小时独立额度；保存的对局、胜局重规划和通过的校验批次复用。来源证明齐备后衔接遗物与选牌联合续局、1536／4608 家庭两组拟合与开发评估。原失败退出和历史证据保留；启动检查点的新增标签与参数更新为 0，未见种子 50% 目标未完成。长任务检查间隔 20 分钟。[恢复登记](docs/experiments/e129-audit-recovery-launch.json)、[当前状态](docs/ironclad-training-status.md)。
