...
**2026-09-20 开发状态：按用户要求暂停原版对齐，E127／E128 沿用冻结的 E121 模拟器训练。** E122 的 6144 局来源采样继续，八进程，20 分钟检查间隔；19:11 检查点保存 2550 个文件。原版对照任务停止，所属进程清理完成。本地衔接进程等待完整来源和模拟器重放证明，随后采集遗物与选牌续局，训练 1536／4608 家庭两组模型并评估。启动检查点的新增标签与参数更新为 0；成绩采用模拟器口径，未见种子 50% 目标未完成。[本轮登记](docs/experiments/e127-e128-simulator-training-launch.json)、[当前状态](docs/ironclad-training-status.md)。
