...
**2026-09-21 范围补充：** 当前采集、训练与验证完成后复盘方案，再选择下一轮。指定 10.17% 父基线的原版 50 局实时评估因接入缺口未启动：完成 0／50，无原版胜率结果；旧实时入口不兼容该模型，胜局回放不能替代自然评估。原版规则对齐继续暂停。[接入检查](docs/experiments/e130-native-baseline-capability.json)。

**2026-09-20 开发状态：模拟器来源校验完成，E127 续局数据采集运行中。** E129 补齐中断的校验，6144 条终局／状态／随机数重放、834427 次局外选择复核、625 条胜局重规划通过。21:50 检查点的遗物续局进度为 617／17552，系统核对确认 8 个工作进程运行；后续衔接选牌组合和 1536／4608 家庭两组拟合。此检查点参数更新为 0。原来源超时失败保留，原版对齐暂停；625／6144 是指定训练与开发家庭的模拟器基线，不代表新模型提升或未见种子验收。[来源完成证明](docs/experiments/e122-e129-source-completion.json)、[当前状态](docs/ironclad-training-status.md)。
