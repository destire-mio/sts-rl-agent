"""Natural-start evaluation of frozen full-run continuous value policies."""
import argparse
...
def independent_choice(policy,checkpoint,gc,observation,actions,descriptors):
    ...
    weights=checkpoint['value_state']
...
def evaluate_worker(job,config):
    try:
        ...
        cp=torch.load(job['checkpoint'],weights_only=True,map_location='cpu');policy=V.ContinuousPolicy(cp,x)
...
def evaluate(root):
    plan=T.registered(root);source=Path(plan['source']);out=root/'evaluation';out.mkdir()
    x=C.D.runtime(plan['runtime']);E.proof(root/'learning','fit-completion.json')
    ...
    rows=x.H.run_jobs(out,jobs,config,'E144_full_runs',deadline,worker_fn=evaluate_worker)
    ...
    reruns=x.H.run_jobs(out,repetitions,config,'E144_winner_replans',deadline,worker_fn=evaluate_worker) if repetitions else []
    ...
    report=dict(status='complete',families=128,arms=reports,natural_policy_evaluation_games=256,
