...
class ContinuousEvaluationTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        model=F.V.ContinuousValue(cls.spec['width'])
        cls.cp=dict(model_type='continuous_fixed_parent_value',feature_spec=cls.spec,
            value_state=model.state_dict(),support=[],base_checkpoint=torch.load(cls.source/'runtime/model.pt',
                weights_only=True,map_location='cpu'))
        cls.policy=F.V.ContinuousPolicy(cls.cp,cls.x)
